...
{	
	...
	public function search(Request $request){
		...
		$option = $request->search_sel;

		if(!$search){
			$serch = NULL;
		}else if($search && $option == 'title'){
			$result = Board::where('board_title', 'like', '%'.$search.'%')->orderby('board_term_open')->get();
		}else if($search && $option == 'place'){
			$result = Board::where('board_place', 'like', '%'.$search.'%')->orderby('board_term_open')->get();
		}else if($search && $option == 'term'){
			$result = Board::where('board_term_open', '<=', $search)->where('board_term_close', '>=', $search)->orderby('board_term_open')->get();
		}
	...
	public function editorUpload(Request $request){
		...
		if($request->file('upload')){
			$file_name = saveImg('upload');
		    $url = asset('img/'.$file_name);
	...
	public function myPage(Request $request){
		...
		$boardCnt = Board::where('board_writer', $user)->count();
		$board = Board::where('board_writer', $user)->get();
		$ticketCnt = Purchase::where('user_id', $user)->count();

		return view('myPage')->with('boardCnt', $boardCnt)
							->with('ticketCnt', $ticketCnt)
							->with('board', $board)
							->with('user', $user);
	}

	public function theaterMaps(){
		return view('theaterMaps');
	}
